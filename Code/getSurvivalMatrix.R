getSurvivalMatrix<-function(clinicalData,type,exceptSet,DataName=NULL){
  clinicalMatrix<-c()
  if(type=="global"){
  ...
  for(i in 1:nrow(clinicalMatrix)){
    if((clinicalMatrix[i,2]=="deceased")||(clinicalMatrix[i,2]=="2"))
      clinicalMatrix[i,2]<-"Dead"
    if((clinicalMatrix[i,2]=="alive")||(clinicalMatrix[i,2]=="1")||(clinicalMatrix[i,2]=="living"))
